...
function HeroSlide({ item }: { item: TVShow }) {
  const title     = item.name;
  const tvUrl     = `/media/tv/${item.id}-${slugify(title)}`;
  const watchUrl  = `/view/tv/${item.id}-${slugify(title)}`;
    ...
    <div className="relative w-full overflow-hidden text-white"
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 px-5 pb-16 md:pb-28 md:px-10 lg:px-16">
        <div className="mb-2 flex items-center gap-2">
          ...
          {year && <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)' }}>{year}</span>}
          {seasons && <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)' }}>{seasons} улирал</span>}
          {rating && <span style={{ fontSize: '11px', color: 'rgba(255,220,50,0.85)', fontWeight: 600 }}>★ {rating}</span>}
        </div>
        ...
        {genres.length > 0 && (
          <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.60)', marginBottom: '14px' }}>
            {genres.join(' · ')}
          </p>
        )}
        ...
        <div className="flex items-center gap-3">
          <Link href={tvUrl} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', minWidth: '48px' }}>
