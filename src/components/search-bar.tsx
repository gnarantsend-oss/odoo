...
// TMDB хайлт нь API key шаарддаг — client-side дуудалтыг /api/search-аар дамжуулна
async function searchTMDB(query: string, type: 'movie' | 'tv', limit = 7): Promise<(Movie | TVShow)[]> {
...
const getSuggestionUrl = (item: Suggestion): string => {
  if ('title' in item && typeof item.title === 'object' && 'type' in item) {
    const title = (item as Media).title.english || (item as Media).title.romaji;
...
const getSuggestionImage = (item: Suggestion): string | null => {
  if ('coverImage' in item) return item.coverImage.large;
  if ('poster_path' in item) return getTMDBImageUrl(item.poster_path);
  return null;
};
...
function SearchBarInternal() {
    ...
    if (debouncedQuery.length > 2) {
      ...
      const fetchSuggestions = async () => {
        try {
          ...
          if (currentTab === 'anime') {
            results = await fetchFromAniList({
              search: debouncedQuery, type: 'ANIME', sort: ['SEARCH_MATCH'], perPage: 7,
            });
          } else if (currentTab === 'manga') {
            results = await fetchFromAniList({
              search: debouncedQuery, type: 'MANGA', sort: ['SEARCH_MATCH'], perPage: 7,
            });
          } else if (currentTab === 'movies') {
            // TMDB: /api/search route-аар дамжуулна (API key browser-т байхгүй)
            results = await searchTMDB(debouncedQuery, 'movie', 7);
          } else if (currentTab === 'tv') {
            results = await searchTMDB(debouncedQuery, 'tv', 7);
          }
  ...
  const getPlaceholder = () =>
    `Search ${currentTab.charAt(0).toUpperCase() + currentTab.slice(1)}...`;
          ...
          >
            <div className="flex flex-col">
              {suggestions.map((item) => {
                ...
                const itemUrl = getSuggestionUrl(item);
...
export function SearchBar() {
  return (
    <Suspense fallback={<div className={cn('h-9 w-full max-w-sm bg-input rounded-md')} />}>
      <SearchBarInternal />
    </Suspense>
  );
}
