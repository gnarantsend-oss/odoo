...
export default function Viewer({
  ...
  const [iframeSrc, setIframeSrc] = useState(() =>
    getIframeSrc(type, mediaId, initialItemNumber, initialSeasonNumber, isDub),
  );
  ...
  const handleNavigation = (newItemNumber: number) => {
    if (newItemNumber < 1) {
      toast({ title: "You're at the beginning!", description: 'This is the first item.' });
      return;
    }
    if (totalItems && newItemNumber > totalItems) {
      toast({ title: "You've reached the end!", description: 'This is the last available item.' });
      return;
    }
        ...
        <div className="flex items-center gap-2">
          {(isTv && media.seasons && media.seasons.length > 1) && (
            <Select
              ...
              defaultValue={seasonNumber.toString()}
            >
      ...
      <main className={cn('relative flex flex-1 items-center justify-center overflow-hidden', isManga ? '' : 'bg-black')}>
        ...
        {iframeSrc && (
          <iframe
            ...
            allowFullScreen
            className={cn('h-full w-full border-0', isLoading ? 'hidden' : 'block', isManga ? 'max-w-4xl' : '')}
